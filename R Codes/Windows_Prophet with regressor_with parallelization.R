...
num_cores <- detectCores() - 6  # Leave one core free to ensure system stability
...
results_list <- foreach(seasonality_prior_scale = seasonality_scale_seq, .combine = rbind) %:%
  foreach(changepoint_prior_scale = changepoint_scale_seq, .combine = rbind) %dopar% {
    ...
    for (i in seq(initial_train_length, nrow(raw_data) - horizon, by = 1)) {
      ...
      # Debugging output before model fitting
      if (any(is.na(training$Fib_supp_MT_log)) || any(is.nan(training$Fib_supp_MT_log))) {
        stop("NA or NaN values found in training data for Fib_supp_MT_log at index: ", i)
      }
      
      # Initialize and fit Prophet model with dynamic parameters and extra regressor
      m <- prophet(
